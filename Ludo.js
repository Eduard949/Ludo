...
function Stuck() {
    ...
    if (onboard[currpawn] == 0||currPos+num>44) {
        if (DontHaveOtherFree()||currPos+num>44) {
            var stucktext = document.getElementById('stucktext');
            stucktext.innerText = "Unfortunatlly you stuck";
            clicked = false;
            var click = document.getElementById('click');
            click.style.backgroundImage = "url(click.gif)";
...
function CheckForWinner() {
    if (pawnOut[currcolor] == 4) {
        ...
        var playerturn2 = document.getElementById("playerturn2");
        click.innerText = "";
        click.style.visibility = "hidden";
        playerturn1.innerText = "";
        playerturn2.innerText = "";
...
function randomNum() {
    if (!clicked) {
        ...
        var click = document.getElementById('click');
        click.style.backgroundImage = "url(" + num + ".jpg)";
        clicked = true;
    }
    if (num != 6&&DontHaveOtherFree()) {
        var stuck = document.getElementById('stucktext');
        stuck.innerText = "Unfortunatlly you stuck";
        window.setTimeout(changePlayer, 1000);
        clicked = false;
    }
}
            ...
            if (text.innerText == currcolor) {
                if (onboard[currpawn] === 1 || num === 6) {
                    if (onboard[currpawn] === 0) {
                        ...
                        switch (Color) {
                            case "red":
                                doc.style.left = 837 + 'px';
                                doc.style.top = 30 + "px";
                                break;

                            case "yellow":
                                doc.style.left = 742 + 'px';
                                doc.style.top = 519 + "px";
                                break;

                            case "blue":
                                doc.style.left = 1035 + 'px';
                                doc.style.top = 324 + "px";
                                break;

                            case "green":
                                doc.style.left = 544 + 'px';
                                doc.style.top = 228 + "px";
                                break;
                        }
                        onboard[currpawn] = 1;
                    }
